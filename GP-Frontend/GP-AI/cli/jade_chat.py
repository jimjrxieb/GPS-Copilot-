    ...
    def __init__(self):
        ...
        self.command_patterns = {
            ...
            r"(scan|check|analyze|test|audit).*project.*quick": {
                "command": "PYTHONPATH=GP-Backend/james-config:$PYTHONPATH ./gp-security scan {project}",
            ...
            r"scan.*advice": {
                "command": "PYTHONPATH=GP-Backend/james-config:$PYTHONPATH ./gp-security advice {project}",
                "description": "Scan with AI advice"
            },
            r"(scan|check).*fix": {
                "command": "PYTHONPATH=GP-Backend/james-config:$PYTHONPATH ./gp-security scan-and-fix {project}",
                "description": "Scan and auto-fix"
            },

            # OPA/Policy commands
            r"(check|validate|test).*policy|opa": {
                "command": "PYTHONPATH=GP-Backend/james-config:$PYTHONPATH python GP-CONSULTING/GP-POL-AS-CODE/2-AUTOMATION/scanners/opa_scanner.py {project} terraform-security",
...
Available commands:
1. PYTHONPATH=GP-Backend/james-config:$PYTHONPATH ./gp-security scan <project> - Quick security scan
2. PYTHONPATH=GP-Backend/james-config:$PYTHONPATH ./gp-security advice <project> - Scan with AI advice
3. PYTHONPATH=GP-Backend/james-config:$PYTHONPATH ./gp-security scan-and-fix <project> - Scan and auto-fix
4. jade query "<question>" - Query knowledge base
5. jade stats - Show system statistics
6. PYTHONPATH=GP-Backend/james-config:$PYTHONPATH python GP-CONSULTING/.../opa_scanner.py <project> terraform-security - OPA policy check
    ...
    def handle_run_fixer(self, user_input: str):
        ...
        for scanner, fixer_path in fixer_map.items():
            ...
            if scan_file.exists():
                ...
                cmd = f"PYTHONPATH=GP-Backend/james-config:$PYTHONPATH python {fixer_path} {scan_file} {project}"
