...
class RAGEngine:
    ...
    def __init__(self):
        # Use the main GP-DATA directory (three levels up from GP-AI to reach root)
        # Path: GP-AI/core/rag_engine.py → GP-AI/core/ → GP-AI/ → GP-Frontend/ → root/
        self.db_path = Path(__file__).parent.parent.parent.parent / "GP-DATA" / "knowledge-base" / "chroma"
    ...
    def init_collections(self):
        ...
        # Scan findings collection (NEW - for auto-sync)
        self.scan_findings = self.client.get_or_create_collection(
            name="scan_findings",
            metadata={"description": "Latest security scan results and findings"}
        )

        # Documentation collection (NEW - for GP-DOCS)
        self.documentation = self.client.get_or_create_collection(
            name="documentation",
            metadata={"description": "Project documentation, reports, and guides"}
        )

        # Project context collection (NEW - for GP-PROJECTS)
        self.project_context = self.client.get_or_create_collection(
            name="project_context",
            metadata={"description": "Project-specific context and metadata"}
        )
